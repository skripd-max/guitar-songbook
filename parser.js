...
export function parseSong(raw){let text=raw.replace(/\r/g,'').replace(/\u00a0/g,' ').trim();const lines=text.split('\n').map(x=>x.trimEnd()).filter((x,i,a)=>!(x===''&&a[i-1]===''));
 let title='',artist='',key='',capo=0;for(let i=0;i<Math.min(lines.length,35);i++){const l=lines[i].trim();let m=l.match(/^(?:title|название|песня)\s*[:—-]\s*(.+)$/i);if(m)title=m[1];m=l.match(/^(?:artist|исполнитель|автор)\s*[:—-]\s*(.+)$/i);if(m)artist=m[1];m=l.match(/^(?:key|тональность)\s*[:—-]\s*([A-G][#b]?m?)/i);if(m)key=m[1];m=l.match(/^(?:capo|каподастр)\s*[:—-]?\s*(\d+)/i);if(m)capo=+m[1]}
 ...
 if(!title){const candidates=lines.slice(0,15).map(x=>x.trim()).filter(x=>x&&x.length<100&&!JUNK.test(x)&&!isChord(x)&&!/[|]{2}/.test(x));title=candidates[0]||'Без названия';artist=candidates[1]||artist}
